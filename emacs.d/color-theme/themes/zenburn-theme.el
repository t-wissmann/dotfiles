...
;; URL: http://github.com/Flowdalic/zenburn-emacs
;; Version: 0.1
...
;; of the new built-in theme support in Emacs 24. This theme is based
;; on two zenburn themes for emacs-24:
;; - Bozhidar Batsov:
;;   https://github.com/bbatsov/zenburn-emacs/blob/master/zenburn-theme.el
;; - Dirk-Jan C. Binnema
;;   https://github.com/djcb/elisp/blob/master/themes/zenburn-theme.el
;;
;; The Zenburn color definitions can be found at
...
(let ( (zenburn-fg-1		"#656555") ; (unused)
       (zenburn-fg		"#dcdccc") ; normal fg

       (zenburn-bg-1		"#2b2b2b")
       (zenburn-bg-05		"#383838")
       (zenburn-bg		"#3f3f3f") ; normal bg
       (zenburn-bg+1		"#4f4f4f")
       (zenburn-bg+2		"#5f5f5f")
       (zenburn-bg+3		"#6f6f6f")

       (zenburn-red+2		"#e37170")
       (zenburn-red+1		"#dca3a3") ; special char
       (zenburn-red		"#cc9393") ; string
       (zenburn-red-1		"#bc8383")
       (zenburn-red-2		"#ac7373")
       (zenburn-red-3		"#9c6363")
       (zenburn-red-4		"#8c5353")
       (zenburn-red-9		"#332323")

       (zenburn-orange		"#dfaf8f")

       (zenburn-yellow+1	"#f8f893")
       (zenburn-yellow		"#f0dfaf") ; conditional
       (zenburn-yellow-1	"#e0cf9f")
       (zenburn-yellow-15	"#dfdfbf")
       (zenburn-yellow-2	"#d0bf8f")

       (zenburn-green-45	"#233323")
       (zenburn-green-4		"#2e3330") ; statusLineNc
       (zenburn-green-1		"#5f7f5f")
       (zenburn-green		"#7f9f7f") ; comment
       (zenburn-green+05	"#88b090")
       ...
       (zenburn-gray-1		"#708070")
       (zenburn-gray-2          "#606060")
       (zenburn-gray-3		"#506070")
       (zenburn-gray-4		"#464646")

       (zenburn-black		"#1e2320")
       (zenburn-white		"#aaaaaa")
  ...
  (custom-theme-set-faces
    ...
    `(zenburn-primary-3 ((t (:foreground ,zenburn-yellow-15 :weight bold))))
    `(zenburn-primary-4 ((t (:foreground ,zenburn-red+1 :weight bold))))
    `(zenburn-primary-5 ((t (:foreground ,zenburn-blue+1 :weight bold))))
    `(zenburn-highlight-damp ((t (:foreground ,zenburn-green+05 :background ,zenburn-green-4))))
    `(zenburn-highlight-alerting ((t (:foreground ,zenburn-red+2 :background ,zenburn-red-9))))
    `(zenburn-highlight-subtle ((t (:background ,zenburn-gray-4))))
    `(zenburn-lowlight-1 ((t (:foreground ,zenburn-gray-2))))
    `(zenburn-lowlight-2 ((t (:foreground ,zenburn-gray-1))))
    ...
    `(header-line ((t (:inherit 'zenburn-highlight-damp
			:box (:color ,zenburn-green-4 :line-width 2)))))
    '(highlight ((t (:weight bold :underline t))))
    `(hover-highlight ((t (:underline t :foreground ,zenburn-yellow+1))))
    '(match ((t (:weight bold))))
    `(menu ((t (:background ,zenburn-black))))
    ...
    `(button ((t (:foreground ,zenburn-yellow :background ,zenburn-gray-3
		   :weight bold :underline t))))
    `(cursor ((t (:background ,zenburn-white :foreground nil))))
    `(escape-glyph-face ((t (:foreground ,zenburn-red))))
    `(minibuffer-prompt ((t (:foreground ,zenburn-yellow))))
    `(mode-line ((t (:foreground ,zenburn-yellow :background ,zenburn-black
		      :box (:color ,zenburn-black :line-width 2)))))
    `(mode-line-inactive ((t (:background ,zenburn-green-4 :foreground ,zenburn-green+05
			       :box (:color ,zenburn-green-4 :line-width 2)))))
    ...
    ;; (Major) Mode configurations start here
    ...
    ;; minimap
    `(minimap-active-region-background ((t (:foreground nil :background ,zenburn-green-45))))
